...
{
    printf("csi_data_print_task started\n");
    ...
    while (xQueueReceive(g_csi_info_queue, &info, portMAX_DELAY))
    {
        ...
        else
        {
            ...
            snprintf(bufDataString_data, sizeof(bufDataString_data), "%d", info->valid_data[0]);
            ...
            {
                uint8_t count_amp = 0;
                for (int i = 52; i < info->valid_len; i += 2)
                {
...
void wifi_csi_raw_cb(const wifi_csi_filtered_info_t *info, void *ctx)
{
    ...
    *q_data = *info;
    ...
    if (g_csi_info_queue == NULL || xQueueSend(g_csi_info_queue, &q_data, 0) == pdFALSE)
    {
...
void radar_config(sensingStruct *sensing)
{
    ...
    BaseType_t task_created = xTaskCreatePinnedToCore(csi_data_print_task, "csi_data_print", 5 * 1024, NULL, 7, &myTaskHandlecsi, 0);
    if (task_created != pdPASS)
    {
        ESP_LOGE(TAG, "Failed to create CSI data print task");
        return;
    }
}
