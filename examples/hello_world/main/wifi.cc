...
// CSI Callback Function (You need to define this function to handle CSI data)
// void wifi_csi_raw_cb(void *ctx, wifi_csi_info_t *csi_info) {
//     // Handle the CSI data here
// }
                          ...
                          int32_t event_id, void *event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        // Enable CSI after getting IP
        ESP_ERROR_CHECK(esp_wifi_set_csi_enabled(true));
        ESP_LOGI(TAG, "CSI enabled");

        // Set the CSI callback function
        ESP_ERROR_CHECK(esp_wifi_set_csi_rx_cb(wifi_csi_raw_cb, NULL));
        ESP_LOGI(TAG, "CSI callback set");
    }
}
